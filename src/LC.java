...
class LC extends JFrame implements KeyListener, ActionListener {
    ...
    *  as MenuBar, MenuItem, ToolBar and Labels etc. */
    ...
    public static JPanel modeSelect, checkInputs;
    ...
    public static JCheckBox hwi; // h/w interrupt
    public static boolean HWI=false;
    public static JCheckBox[][] cb = new JCheckBox[24][8]; // IO port checkboxes
    ...
    * It relates the button and menu item closely. */
    ...
    public AbstractAction checkAction = new FileEditMenuAction("",null,"",null,null, frame);
...
//        setSize (FRAME_WIDTH, FRAME_HEIGHT);
        ...
        JPanel contentPanec = new JPanel();
        contentPanec.setLayout(new BoxLayout(contentPanec,BoxLayout.PAGE_AXIS));
        for(int i=0;i<24;i++){
        	for(int j=0;j<8;j++){
        		cb[i][j]= new JCheckBox(checkAction);
        		cb[i][j].setIcon(new ImageIcon(LC.class.getResource("0.png")));
        		cb[i][j].setSelectedIcon(new ImageIcon(LC.class.getResource("1.png")));
        		cb[i][j].setActionCommand("check "+i);	
        	}
        }
        JPanel[] cbLine = new JPanel[24];
        for(int i=0;i<24;i++){
        	cbLine[i] =  new JPanel();
        	cbLine[i].setLayout(new BoxLayout(cbLine[i],BoxLayout.LINE_AXIS));	
        	String lab = "";
        	if(i<10) lab = "0";
        	cbLine[i].add(new JLabel("   Port #"+lab+""+i+"         "));
        	for(int j=0;j<8;j++){
        		cbLine[i].add(cb[i][j]);
        	}
        	contentPanec.add(new JSeparator());
        	contentPanec.add(cbLine[i]);
        }
        JScrollPane scrollArea1  = new JScrollPane(contentPanec,ScrollPaneConstants.VERTICAL_SCROLLBAR_ALWAYS,ScrollPaneConstants.HORIZONTAL_SCROLLBAR_NEVER);     
        ...
        */
        ...
        IOdevicePanel.setBounds(660,545,355,140);
        ...
        // No need to add in this version 2.0.
    ...
    */
    ...
    JToolBar setUpToolBar() {
        ...
        hwi = new JCheckBox("");
        hwi.setIcon(new ImageIcon(LC.class.getResource("HWI1.png")));
		hwi.setSelectedIcon(new ImageIcon(LC.class.getResource("HWI2.png")));
		hwi.setBorder(BorderFactory.createLineBorder(Color.black));
		hwi.setEnabled(false);
        hwi.setActionCommand("hwi");
