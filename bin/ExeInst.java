...
public class ExeInst extends JPanel {
    ...
    */
    public void controlUnit(Boolean overRide) {
        System.out.println("in controll unit");
...
//            System.out.println(MD);
           ...
           // System.out.println(IR);
            ...
            LC.HWI=false;
            // intrupt bit set
            if(LC.hwi.isSelected()){
            	LC.HWI=true;
            	LC.hwi.setEnabled(false);
            	IR = "A0000000";
            	incrPC(-4);
            	LC.writeMessage("Hardware Interrupt.");
            }
...
//        System.out.println("in "+in);
        if(in.substring(0,2).equals("00")) {
            ...
            if(instrRec.equals("000")) {
                ...
                // CALL instruction
                int SPreg = 15; // r15 stack pointer
                incrPC(4);
                ...
                if(tempSP<4091){
                	...
                	LC.writeMessage("No more call allowed.");
                  // incremented SP 
            }
                ...
                if(in.charAt(5)=='0') {
                    ...
                    incrPC(1);
                    int x;
                    System.out.println("trying to set halt state");
                    LC.halt = true;
                }
            }
            ...
            if(inOut.equals("1110")) {
                ...
                if (portno>=0&&portno<=23 ) {    // simulator has only ports 0 to 23
                    String val = (String) LC.IOdevicePanel.table.getValueAt(portno/2,1+2*(portno%2));
...
//                    LC.IOdevicePanel.table.setValueAt(HexToBinary(MD.substring(6),8),portno/2,1+2*(portno%2));
                ...
                if (portno>=0&&portno<=23 ) {     // simulator has only ports 0 to 23
                    ...
                    if(portno>=0 && portno<4){
                    	String bin = HexToBinary(MD.substring(6),8);
                    	for(int i=0;i<8;i++){
                    		if(bin.charAt(i)=='0') LC.cb[portno][i].setSelected(false);
                    		else LC.cb[portno][i].setSelected(true);
                    	}
        ...
        if(opcode.charAt(7)=='0') {
            ...
            if(opcode.charAt(6)=='1') {
            	...
            	if(bitsToShift>7){
	                ...
	                valToShift=ones+valToShift.substring(tot_shift, 8);
	                val=Integer.parseInt(valToShift,2);
            	}
                t = CustTable.pad(Integer.toHexString(val),-2,"0");
            }
    ...
    public void shiftWord( int bitsToShift ) {
        ...
        if(opcode.charAt(7)=='0') {
            ...
            if(opcode.charAt(6)=='1') {
                ...
                valToShift=ones+valToShift.substring(tot_shift, 32);
                val=Long.parseLong(valToShift,2);
